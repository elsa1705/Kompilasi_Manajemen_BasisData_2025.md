...
## 1. Latar Belakang
Dalam sistem manajemen basis data, bottleneck dapat menyebabkan penurunan performa dalam eksekusi query, terutama pada tabel besar dengan jumlah data yang terus bertambah. Optimasi bottleneck dalam MySQL diperlukan untuk meningkatkan efisiensi akses data dengan memanfaatkan teknik seperti indexing, query optimization, dan penggunaan query cache. Praktikum ini bertujuan untuk memahami cara mengidentifikasi dan mengatasi bottleneck dalam database guna meningkatkan performa sistem.
...
## 4. Pembahasan
Optimasi dilakukan dengan menambahkan indeks pada kolom yang sering digunakan dalam kondisi WHERE dan JOIN, seperti customer_id, kode_item, dan tgl_transaksi. Penggunaan EXPLAIN dalam MySQL membantu dalam mengevaluasi apakah indeks dimanfaatkan secara optimal. Selain itu, optimasi dilakukan dengan merancang ulang query agar lebih efisien, seperti mengganti LIKE '%keyword%' dengan pencarian berbasis indeks atau menggunakan teknik partisi tabel untuk membagi data dalam skala besar. Hasil optimasi menunjukkan peningkatan signifikan dalam waktu eksekusi query, membuktikan bahwa bottleneck dapat diminimalkan dengan penerapan indeks dan strategi query yang lebih optimal.

## 5. Kesimpulan
Dari hasil optimasi, dapat disimpulkan bahwa bottleneck dalam MySQL dapat diatasi dengan menerapkan indeks yang tepat, merancang query yang lebih efisien, serta memanfaatkan query cache untuk mempercepat eksekusi query yang berulang. Teknik ini secara signifikan meningkatkan performa database, mengurangi beban server, dan mempercepat akses data.
